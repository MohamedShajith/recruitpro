...
def create_mobilization(doc,method):
    if doc.pending_for == 'Proposed PSL':
        ...
        mobilization_id = frappe.db.exists("Mobilization", {"candidate": doc.name})
        if mobilization_id:
        ...
        if doc.irf:
            mobilization.update({"irf": doc.irf})
        if doc.candidate_image:
            mobilization.update({"photo": doc.candidate_image})
        if doc.passport:
            mobilization.update({"passport": doc.passport})
        if doc.offer_letter:
            mobilization.update({"offer_letter": doc.offer_letter})
        mobilization.flags.ignore_mandatory = True
