                                    ...
                                    if self.visa and self.so_created:
                                        ...
                                        if self.is_required:
                                            if not self.certificate_attestation:
                                                frappe.db.set_value(
                                                    "Mobilization", self.name, "status", "Certificate Attestation")
                                                # self.save(ignore_permissions=True)
                                                frappe.reload_doctype("Mobilization")
                                            else:
                                                self.status = 'Visa'
...
@frappe.whitelist()
def create_sale_order(mobilization, project, customer, position, candidate_name, contact, payment,client_sc,candidate_owner,sa_id,candidate_sc,billing_currency, territory, associate,passport_no, expected_doj, delivery_manager, account_manager,service,supplier,associate_si,client_si,candidate_si,candidate_dec,dec):
    ...
    if payment:
        ...
        if item_candidate_id or item_pp_id:
            ...
            item.default_unit_of_measure = "Nos"
            item.qty = "1"
            ...
            if territory != 'India' or parent_territory != 'India':
                if payment == "Client":
                    ...
                    so.append("items", {
                        ...
                        "uom": item.default_unit_of_measure,
                        "is_stock_item": "0",
                ...
                if payment == "Both":
                    ...
                    frappe.db.commit()
                    # so.submit()
                    frappe.set_value('Mobilization', mobilization, 'status', 'Visa')
            ...
            frappe.set_value('Mobilization', mobilization, 'so_created', 1)
            frappe.set_value('Mobilization', mobilization, 'so_confirmed_date', today())
